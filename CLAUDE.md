...
- `fzf` is bound by the same rule. `FZF_DEFAULT_OPTS` (in `.bashrc`) pins `--color=16`, which restricts fzf to the 16 ANSI slots so its menu rides the Modus palette and follows the `Ctrl+Shift+t` toggle. Left to itself fzf draws with 256-colour codes and ignores the theme entirely. Most `FZF_DEFAULT_OPTS` snippets found online spell out `--color=fg:#...,bg:#...` pairs — pasting one in hardcodes RGB outside `foot.ini` and breaks the toggle, so keep `--color=16` if you extend that variable.
